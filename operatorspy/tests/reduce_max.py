...
def reduce_max(input, axis, keepdims=True, noop_with_empty_axes=False):
    if axis == ():
...
def test(
    ...
    print(
        f"Testing ReduceMax on {torch_device} with data_shape:{data_shape}  axes:{axes}  keepdims:{keepdims}  noop_with_empty_axes:{noop_with_empty_axes}  dtype:{tensor_dtype}"
    )
    ...
    ans = reduce_max(data, axes, keepdims, noop_with_empty_axes)
    ...
    if PROFILE:
        ...
        for i in range(NUM_ITERATIONS):
            _ = reduce_max(data, axes, keepdims, noop_with_empty_axes)
...
if __name__ == "__main__":
    test_cases = [
        ...
        # ((2, 3, 2, 5), (-3, -2), True, False),  #Floating point exception
        # ((3, 2, 5, 4), (-4, -3, -2, -1), True, False),
