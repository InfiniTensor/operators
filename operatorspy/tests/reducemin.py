...
def reduce_min(input, axis, noop_with_empty_axes, keepdims=True):
    if axis == None:
...
def test(
    ...
    print(
        f"Testing reducemin on {torch_device} with x_shape:{x_shape} dtype:{tensor_dtype}"
    )
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        ans = reduce_min(x, axes if dynamic_axes == None else dynamic_axes, noop_with_empty_axes, keepdims)
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            _ = reduce_min(x, axes if dynamic_axes == None else dynamic_axes, noop_with_empty_axes, keepdims)
