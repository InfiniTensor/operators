...
def reduce_min(input, axis, keepdims=True, noop_with_empty_axes=False):
    if axis == ():
        if noop_with_empty_axes:
            return input
        else:
            return torch.amin(input, dim=axis, keepdim=keepdims)
    return torch.amin(input, dim=axis, keepdim=keepdims)
...
def test(
    ...
    print(
        f"Testing ReduceMin on {torch_device} with data_shape:{data_shape}  axes:{axes}  keepdims:{keepdims}  noop_with_empty_axes:{noop_with_empty_axes}  dtype:{tensor_dtype}"
    )
    ...
    ans = reduce_min(data, axes, keepdims, noop_with_empty_axes)
    ...
    if PROFILE:
        ...
        for i in range(NUM_ITERATIONS):
            _ = reduce_min(data, axes, keepdims, noop_with_empty_axes)
